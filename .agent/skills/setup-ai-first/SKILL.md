...
Copy the template and instruct: "Open `.agent/project-context.yaml` and fill in all `{{PLACEHOLDER}}` values. For Python projects, recommended values:
  - `language`: Python 3.11 (or 3.10, 3.12)
  - `framework`: FastAPI (or Django, Flask, Starlette)
  - `build_tool`: Poetry (or pip, pdm, uv)
  - `test_framework`: pytest (or unittest)
  - `module`: python-fastapi (or python-kafka, python-django, context-engineering)
